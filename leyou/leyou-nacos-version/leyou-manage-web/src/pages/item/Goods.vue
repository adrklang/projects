    ...
    methods: {
      deleteGoods(id){
        this.$message.confirm("确认删除商品?").then(() =>{
          this.$http.delete("/item/goods/" + id).then(result =>{
          ...
          this.$message.info("取消删除商品成功")
        })

      },
      soldOut(item){
        this.$http.put("/item/goods/" + item.id + "?state=soldOut").then(result =>{
      ...
      putaway(item){
        this.$http.put("/item/goods/" + item.id + "?state=putaway").then(result =>{
